package sizzle.easing {
	public class Elastic {
		...
		public static function easeIn(p:Number) {
			return Math.sin(13 * _PI_2 * p) * Math.pow(2, 10 * (p - 1));
		}

		// Modeled after the damped sine wave y = sin(-13pi/2*(x + 1))*pow(2, -10x) + 1
		public static function easeOut(p:Number):Number {
			return Math.sin(-13 * _PI_2 * (p + 1)) * Math.pow(2, -10 * p) + 1;
		}
		...
		public static function easeInOut(p:Number):Number {
			if (p < 0.5) {
				return 0.5 * Math.sin(13 * _PI_2 * (2 * p)) * Math.pow(2, 10 * ((2 * p) - 1));
			}
			else {
				return 0.5 * (Math.sin(-13 * _PI_2 * ((2 * p - 1) + 1)) * Math.pow(2, -10 * (2 * p - 1)) + 2);
			}
		}
	}
}